...
One write-path consequence worth knowing: the self-correcting NLI write-gate runs on every store, so a teammate storing a near-twin of your note *can* auto-retire yours when the model reads the pair as a bidirectional contradiction. Heavily templated notes (shared boilerplate, one variable changed) are the false-positive risk. Every auto-retire is audited in `memory_conflicts` and recoverable (`memory_history`, `as_of`); set `MCP_NLI_DISABLED=1` to turn the gate off for such corpora (see `docs/ENV.md`).
