...
- **`as_of` of a SUPERSEDED fact only works via `keyword`/`hybrid`, not `vector`.** (verified, known limitation) `invalidateMemory` drops the retired fact's vec row, but the candidate set is built from the vec/FTS indexes *before* the `as_of` predicate applies — so a `search_mode:'vector'` point-in-time query returns nothing for retired facts (the FTS row survives, so keyword/hybrid with matching terms still find them). For reliable point-in-time recall of a retired fact use `keyword`/`hybrid` mode, or `memory_history` by id.
...
- **Soft-forget is "recoverable" but has no un-tombstone tool.** (verified, known limitation) `memory_forget {hard:false}` stamps `valid_to`; `memory_update`/`memory_restore` do NOT clear it. The row + content survive and stay `as_of`-queryable, but reinstating it into default recall currently needs direct DB access.
...
- **vec0 rows can survive bitemporal invalidation.** `invalidateMemory` drops the vec row on the store-supersede and `memory_forget` paths, but `recordConflicts`' heuristic supersede stamps `valid_to` **without** dropping it — any new direct `memories_vec` consumer must re-filter `valid_to`/`tx_expired` in JS. (This asymmetry is also why `as_of` vector reconstruction of retired facts fails — see §1.)
- **`entity_aliases` is write-only.** (verified) `memory_extract_entities` stores aliases + bumps `aliases_added`, but no tool (graph/search/query/pagerank) resolves an alias → entity. Merge entities by giving them the SAME name spelling (`normalizeName` collapses `Node.js`/`nodejs`/`node js` → one), not by aliasing.
