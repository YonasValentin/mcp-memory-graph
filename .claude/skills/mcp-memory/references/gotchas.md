...
- **`entity_aliases` resolution is `memory_graph`-only** (fixed for graph). `memory_extract_entities` stores aliases; **`memory_graph` now resolves a normalized alias → its entity** (direct entity-name match wins). But `memory_search`/`memory_query`/PageRank still match on `entity`/`normalized_name` and do NOT expand aliases — for those, merging entities by the SAME spelling (`normalizeName` collapses `Node.js`/`nodejs`/`node js` → one) is still the reliable path.
