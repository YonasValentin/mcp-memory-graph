...
| `memory_list` | Browse/paginate (no query). Bitemporal-filtered. Returns `{ items, total, limit, offset, has_more }` — read rows from **`.items`** (NOT `.results`). Does NOT filter `expires_at`. | scope, namespace, sort_by, sort_order, as_of, limit, offset | ✅ `/api/memories` |
