...
```bash
npx mcp-memory-server vault-init --vault ~/team-vault   # writes .gitattributes binding the union-merge driver — REQUIRED
```
...
npx mcp-memory-server sync --vault ~/team-vault   # flush DB → vault, write the graph sidecar
git add -A && git commit -m "memory: ..." && git push   # stages the vault .md + .memory/graph.json sidecar + .gitattributes
# teammate:
git pull
npx mcp-memory-server rebuild --vault ~/team-vault   # rebuild the SQLite cache from the .md files
```
...
**Setup:**
```bash
npx mcp-memory-server init                  # user scope → ~/.claude/settings.json (all projects)
npx mcp-memory-server init --scope project  # → .claude/settings.json + .mcp.json (collaborators auto-discover)
npx mcp-memory-server init --yes            # accept defaults, non-interactive
npx mcp-memory-server uninstall             # reverse
```
