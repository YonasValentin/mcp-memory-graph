...
- **REST/MCP write path is NOT namespace-forced** — `memory_store` registration omits `withForcedNs` (reads have it). Latent multi-tenant write-isolation gap; moot single-tenant.
- **Corrupt/non-numeric `schema_version` silently skips ALL migrations** (no error, no climb) — a robustness gap in `runMigrations` (P11).
- **`logger` redaction is shallow + key set incomplete** (`access_token`/`client_secret` leak); nested frontmatter `__proto__` survives as an own-enumerable key (only a naive deepMerge would weaponize it) (P8).
- **Native-module teardown crash:** scripts holding a sqlite-vec DB connection open at `process.exit()` abort with `mutex lock failed` (exit 134) — close the DB before exit (P10/P14).
- **`memory_query` does not alias-expand** — its seed `hybridSearch` runs with `use_graph` off (original design), so alias resolution doesn't reach it. Enabling `use_graph` there would wire it in but changes seed ranking/latency for every call — a deliberate separate decision, not done.
