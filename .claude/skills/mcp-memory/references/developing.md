...
A **14-persona battle wave v3** then drove the REAL compiled handlers at this HEAD (a brand-new solo user + a multi-dev team + 12 skeptics, real models, /tmp throwaway, adversarial refute-verify + completeness critic): **all 5 fixes above HOLD**; newcomer succeeded from the skill alone; team primitives + the `agent_id`/`access_level` round-trip (9ffbc9d) + F1/F1b write-isolation all verified end-to-end (incl. P10's real `buildApp`+`POST /mcp` server path). Zero blockers/majors. It also surfaced the P11 residual (now fixed) + the OPEN items below.

Genuinely **open** (verified by the persona pass unless noted):
- **Real-embedder teardown still aborts exit 134** (P14) — the ONNX/transformers.js native worker mutex-crashes on `process.exit()` even with NO DB open; `closeAllDatabases()` does not cover it. Every real-embedder CI/battle script exits non-zero, which can MASK a real failure. Fix candidate: dispose the pipeline before exit, or mandate the mock embedder in force-exit scripts. Long-running `serve` is unaffected (never force-exits). Related: a dual-connection-to-one-WAL-file + `process.exit()` (injected `createDatabase` + cached `getReadWriteDb`) still 134s (P10) — production `runServe` shares one connection, so it's a harness-only footgun.
- **`SQLITE_BUSY` on write-upgrade under concurrency** (P9, deterministic) — read-then-write txns (`deleteMemory`/`invalidateMemory`/`updateMemory`/consolidate-prune) use better-sqlite3's **deferred** `BEGIN`, so a write-upgrade while another writer is active throws `SQLITE_BUSY` in 0ms (busy_timeout NOT honored); `consolidate` swallows it and **silently under-prunes**. No corruption — availability only. Fix: `BEGIN IMMEDIATE` (`.immediate()`) on write-after-read txns. Bites the team/multi-agent shared-DB path, benign for single-user.
