...
- **fix(migrations): throw on corrupt `schema_version`** (P11) — `parseInt('garbage')`→NaN made every `m.version > NaN` false → `pending=[]` → silently applied ZERO migrations against an un-migrated schema. Strict `Number(...)` + `Number.isInteger`/≥0, throws a clear error; benign paths (missing→0, `'0'` seed, valid ints) unchanged. Fail-closed.
- **fix(connection): close `createDatabase` survivors at exit** (P10/P14) — uncached connections were untracked, so a sqlite-vec DB left open at `process.exit()` aborted `mutex lock failed` (134). `Set<WeakRef>` + `FinalizationRegistry` (leak-free — stress-verified GC pruned 2000/2000), `.close()` deregisters, `closeAllDatabases()` wired into the exit handler. **`npm run smoke` now exits cleanly** (the real-embedder teardown previously crashed — this is what blocked unit-testing the live `createServer` store path).
- **fix(logger): recursive secret redaction** (P8) — `redact()` matched only top-level keys and missed `access_token`/`refresh_token`/`client_secret`/`private_key`/`session_token`/`bearer`; nested/array secrets logged cleartext and a circular input threw at `JSON.stringify`. Now recursive over objects+arrays with a WeakSet circular guard + `MAX_DEPTH=6`, own-enumerable keys only (no `__proto__`). Mirrors `sanitizeDeep`.
