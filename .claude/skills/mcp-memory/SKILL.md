---
name: mcp-memory
description: Expert operation AND development of mcp-memory-graph — a local-first, bi-temporal, knowledge-graph memory server for Claude Code (50 MCP tools over one SQLite file + local embeddings; $0/token, no cloud, no telemetry). Use when (1) DRIVING the memory tools in any project wired to the server — storing/recalling/searching memories, choosing among the 50 memory_*/vault_*/core_memory_* tools, setting up auto-capture hooks, running a team git-vault sync, GDPR forget, the dream-cycle consolidate, or debugging why a memory is not found (scope/privacy/rerank surprises); or (2) DEVELOPING the server itself inside the mcp-memory-graph repo — its architecture, where to change code, the build/test/bench/smoke commands, verified gotchas and tech debt, or the REST/Docker/self-hosted deploy. Triggers include any use of mcp__*memory*__ tools, "remember this"/"recall that", "store a decision", Obsidian vault sync, core memory, knowledge graph of memories, or work inside the mcp-memory-graph codebase.
---
