...
**Public benchmarks (4/4 parity with MemPalace, untuned) — [LongMemEval-S](https://github.com/xiaowu0162/LongMemEval) R@5 = 97.8% (vs 96.6%); [ConvoMem](https://huggingface.co/datasets/Salesforce/ConvoMem) R@10 = 93.5% (vs 92.9%); [LOCOMO](https://github.com/snap-research/locomo) session R@10 = 82.2%, R@50 = 100% (vs 60.3% baseline); [MemBench](https://github.com/import-myself/Membench) hit@5 = 78.7% (vs their 80.3% *tuned*)** — production handlers, stock MiniLM embedder, zero benchmark-specific tuning, fully local, $0/token. Run them yourself: `npm run bench:longmemeval` / `bench:locomo` / `bench:convomem` / `bench:membench`. Full tables + honest notes (incl. where the reranker helps vs hurts, and the dedup floor on MemBench) are in [`docs/BENCHMARKS.md`](docs/BENCHMARKS.md).
