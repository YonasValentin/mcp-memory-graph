...
- **`memory_consolidate`** — the dream cycle: Score → Expire → Prune → Dedup → Gaps. `dry_run:true` FIRST. `similarity_threshold` (0.85), `prune_expired`, `prune_low_quality`, `max_operations`, scope/namespace limits.
- **`memory_extract_learnings`** — heuristically mine a transcript for decisions/patterns/error_fixes/conventions; `auto_store` (true).
